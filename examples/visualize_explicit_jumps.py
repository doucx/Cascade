...
def create_cicd_workflow():
    ...
    lint_selector = cs.select_jump({"needs_fix": fixer_run, None: tester_run})

    # Test 失败跳转到 Fix，成功则结束
    test_selector = cs.select_jump({"needs_fix": fixer_run, None: None})

    # Fix 之后跳回 Lint 重新开始
    fix_selector = cs.select_jump({"retry": linter_run})
...
if __name__ == "__main__":
    ...
    print(
        "或者在本地运行: python examples/visualize_explicit_jumps.py | dot -Tpng -o workflow.png"
    )
    print("=" * 40)
