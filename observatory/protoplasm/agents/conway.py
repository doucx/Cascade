...
async def collect_neighbors(
    ...
    async def callback(topic: str, payload: Any):
        ...
        if sender is None or p_gen is None:
            return
    ...
    sub = await conn.subscribe("cell/+/state", callback)
    try:
        await asyncio.wait_for(
            future, timeout=10.0
        )  # Generous timeout for high-concurrency stress
    except asyncio.TimeoutError:
        raise RuntimeError(
            f"Agent timed out waiting for gen {current_gen}. Mailbox state: {current_mb.get(current_gen)}"
        )
    finally:
...
def conway_agent(
    ...
    def lifecycle(gen: int, current_state: int, mailbox: Dict[int, Dict[int, int]]):
        ...
        b_act = broadcast_state(
            topic_base, agent_id, gen, current_state, connector, rendezvous=rendezvous
        )
        r_act = report_to_validator(
            validator_topic,
            agent_id,
            x,
            y,
        ...
        collected_data = collect_neighbors(
            gen, mailbox, neighbor_ids, connector, rendezvous=rendezvous
        )
