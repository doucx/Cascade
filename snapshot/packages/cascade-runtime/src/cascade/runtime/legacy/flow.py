...
from cascade.runtime.graph.model import Node, Graph, EdgeType, Edge
...
class FlowManager:
    ...
    async def _process_router_decision(
        ...
        for route_key, route_lazy_result in router.routes.items():
            if route_key != selected_route_key:
                ...
                branch_root_id = branch_root_node.current_node_instance_hash
    ...
    async def should_skip(
        ...
        if router_edge and router_edge.router:
            ...
            if selector_node:
                selector_id = selector_node.current_node_instance_hash
                    ...
                    for upstream_edge in self.in_edges[
                        ...
                        if (
                            ...
                            and await state_backend.has_result(
                                upstream_edge.source.current_node_instance_hash
                            )
                        ):
                            can_penetrate = True
                            break
